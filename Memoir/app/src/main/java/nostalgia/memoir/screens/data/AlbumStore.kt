...
fun renameAlbum(context: Context, albumId: String, newName: String) {
    val myAlbums = loadMyAlbums(context)
    val sharedAlbums = loadSharedAlbums(context)

    val inMy = myAlbums.any { it.id == albumId }
    if (inMy) {
        saveMyAlbums(context, myAlbums.map { if (it.id == albumId) it.copy(name = newName) else it })
    } else {
        saveSharedAlbums(context, sharedAlbums.map { if (it.id == albumId) it.copy(name = newName) else it })
    }
}
